...
test("mở gói chỉ công bố kết quả ở đúng tab KQLCNT", () => {
  ...
  assert.equal(url.searchParams.get("notifyId"), "6a1a1c46-a831-4c91-bc42-acde01343210");
  assert.equal(
...
test("gói thiếu UUID dùng trang tra cứu chính thức thay vì link detail bị lỗi", () => {
  const item = {
    id: "IB2600525594",
    notifyId: "IB2600525594",
    notifyNo: "IB2600525594",
    inputResultId: "8203bd1a-2ade-46c5-b79b-81a964a78fd2",
  };
  assert.equal(hasOfficialDetailId(item), false);
  assert.equal(buildOfficialSourceUrl(item), officialSearchUrl());

  const broken = "https://muasamcong.mpi.gov.vn/web/guest/contractor-selection"
    + "?_egpportalcontractorselectionv2_WAR_egpportalcontractorselectionv2_render=detail-v2"
    + "&id=IB2600525594&notifyId=IB2600525594&notifyNo=IB2600525594";
  assert.equal(repairOfficialSourceUrl(broken), officialSearchUrl());
});
...
test("không ghi chuỗi null hoặc undefined vào link", () => {
  const sourceUrl = buildOfficialSourceUrl({
    id: "6a1a1c46-a831-4c91-bc42-acde01343211",
    inputResultId: "null",
    bidOpenId: undefined,
    techReqId: "undefined",
  });
...
test("sửa link cũ đang ép kết quả lựa chọn nhà thầu về tab TBMT", () => {
  const oldUrl = buildOfficialSourceUrl({
    id: "6a1a1c46-a831-4c91-bc42-acde01343212",
    notifyId: "6a1a1c46-a831-4c91-bc42-acde01343212",
    inputResultId: "8203bd1a-2ade-46c5-b79b-81a964a78fd2",
